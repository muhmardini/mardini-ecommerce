...
const SingleProduct = () => {
  ...
  return (
    <main className="flex flex-col md:flex-row py-24 min-h-screen justify-between items-start md:items-center w-full">
      <div className="flex flex-col md:flex-1 px-8">
        ...
        <div className="md:flex md:flex-1 hidden">
          ...
          <div className="flex-1 flex flex-col justify-center items-center border-l">
            ...
            />
            <img
              className="flex-1  w-30 hidden md:block"
              src={product?.images[1]}
              alt={product?.title}
            />
          </div>
      ...
      <div className="w-px bg-basic h-120 hidden md:block md:mx-10"></div>
      <div className="flex-1 self-start mt-12 flex flex-col gap-y-4 px-4">
        <h1 className="text-secondary">{product?.title}</h1>
        <p className="text-wrap md:text-[32px] text-xl mt-4">{product?.description}</p>
        <div>
        ...
        <div className="flex items-center gap-6 justify-center">
          <button
            className="btn btn-primary md:px-12 px-8 py-2 md:text-xl text-sm shadow-lg"
            type="button"
          >
            Buy Now
          </button>
          <button
            className="btn btn-secondary md:px-12 py-2 px-8 md:text-xl text-sm shadow-xl"
            type="button"
          >
            Add to Cart
          </button>
