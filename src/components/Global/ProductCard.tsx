...
export const ProductCard = ({ data, location }: productsData) => {
  ...
  if (!data) return null;
    ...
    if (location.toLowerCase() === "/") {
      return (
        <motion.div variants={container2} viewport={{once:true, amount: 0.2}} layout initial="hidden" whileInView="visible" className="min-w-60 border border-b-basic p-1 rounded-2xl flex flex-col ml-2 transform-scale cursor-pointer">
